...
-- Tìm tất cả cách sách thuộc thể loại Khoa hoc
SELECT * FROM Books b WHERE b.Genre = 'Khoa học';

-- Liệt kê các thành viên sống tại thành phố “Hanoi”
SELECT * FROM Members m WHERE m.City = 'Hanoi';

-- Tìm các sách xuất bản sau năm 2015
SELECT * FROM Books b WHERE b.YearPublished > 2015;
...
-- Liệt kê tên các thành viên và số lần họ mượn sách
SELECT m.Name, COUNT(*) SoLanMuonSach
FROM Members m
JOIN Borrowings br ON m.MemberID = br.MemberID
GROUP BY m.Name;

-- Tìm những sách chưa từng được mượn
SELECT *
FROM Books b
LEFT JOIN Borrowings br ON b.BookID = br.BookID
WHERE br.BorrowID IS NULL;

-- Liệt kê các thành viên mượn hơn 5 cuốn sách
SELECT m.Name, COUNT(*) as SoLanMuonSach
FROM Members m
JOIN Borrowings br ON m.MemberID = br.MemberID
GROUP BY m.Name
HAVING SoLanMuonSach > 5; -- Dung WHERE dua tren du lieu co san cua 2 bang, sau do muon ap dung dieu kien len du lieu 1 lan nua thi dung HAVING

-- Tìm thành viên có tổng số ngày giữ sách nhiều nhất
SELECT m.Name, SUM(DATEDIFF(br.ReturnDate, br.BorrowDate)) TongSoNgayMuonSach
FROM Members m JOIN Borrowings br ON m.MemberID = br.MemberID
GROUP BY m.Name
ORDER BY TongSoNgayMuonSach DESC
LIMIT 1;

-- Liệt kê tên thành viên và số sách đã mượn kèm số sách chưa trả
SELECT m.Name, COUNT(br.BookID) AS SoSachDaMuon, SUM(br.BorrowDate IS NOT NULL AND br.ReturnDate IS NULL) AS SoSachChuaTra
FROM Members m
